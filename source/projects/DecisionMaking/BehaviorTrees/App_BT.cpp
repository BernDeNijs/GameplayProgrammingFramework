...
void App_BT::Start()
{
	...
	auto* pBehaviorTree = new BehaviorTree(pBlackBoard,
		
		
		new BehaviorSelector //ROOT
		({
			...
			({
				...
				new BehaviorNotConditional(BT_Conditions::IsSearchingTooLong),
				new BehaviorPartialSequence({
					new BehaviorAction(BT_Actions::SeekSearchpoint),
					new BehaviorAction(BT_Actions::SearchAround),
})
				}),
...
Elite::Blackboard* App_BT::CreateBlackboard(SmartAgent* smartAgent)
{
	...
	pBlackboard->AddData("PatrolBehavior", m_pGuardPathFollow);
	pBlackboard->AddData("DetectionRadius", m_DetectionRadius);
	pBlackboard->AddData("LastKnownLocation", Vector2{});
	pBlackboard->AddData("TimeLastSeen", milliseconds{});
	pBlackboard->AddData("MaxSearchTime", m_MaxSearchTime);
	pBlackboard->AddData("PlayerSpottedRecently", bool{false});
