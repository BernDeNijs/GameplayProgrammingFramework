...
void App_FSM::Start()
{
	...
	m_pPatrolState = new ::FSMState::PatrolState();
	m_pChaseState = new ::FSMState::ChaseState();
	m_pSearchState = new ::FSMState::SearchState();


	//3. Create the transitions between those states
	m_pIsTargetVisible = new ::FSMCondition::IsTargetVisible();
	m_pIsTargetNotVisible = new ::FSMCondition::IsTargetNotVisible();
	...
	pFSM->AddTransition(m_pPatrolState, m_pChaseState, m_pIsTargetVisible);
	pFSM->AddTransition(m_pChaseState, m_pSearchState, m_pIsTargetNotVisible);
	pFSM->AddTransition(m_pSearchState, m_pChaseState, m_pIsTargetVisible);
	pFSM->AddTransition(m_pSearchState, m_pPatrolState, m_pIsSearchingTooLong);
	...
	//6. Activate the decision making structure on the custom agent by calling the SetDecisionMaking function
	m_pGuard->SetDecisionMaking(pFSM);
...
Elite::Blackboard* App_FSM::CreateBlackboard(SmartAgent* smartAgent)
{
	...
	pBlackboard->AddData("SearchTime", 0.0f);
	pBlackboard->AddData("DetectionRadius", m_DetectionRadius);
